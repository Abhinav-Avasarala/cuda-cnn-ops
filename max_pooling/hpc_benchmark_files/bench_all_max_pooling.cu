...
// CUDA implementation in ../max_pooling.cu; pointers are device pointers.
...
static void bench_one(int N, int C, int H, int W, int K, int S, int P,
  ...
  std::vector<float> out_gpu((size_t)N * C * H_out * W_out, 0.0f);
  ...
  cuda_check(cudaMalloc(&d_input, sizeof(float) * input.size()), "cudaMalloc d_input");
  cuda_check(cudaMalloc(&d_output, sizeof(float) * out_gpu.size()), "cudaMalloc d_output");
  cuda_check(cudaMemcpy(d_input, input.data(), sizeof(float) * input.size(), cudaMemcpyHostToDevice), "H2D input");

  // Kernel-only timing; solve() synchronizes internally.
  auto run_cuda = [&]() { solve(d_input, d_output, N, C, H, W, K, S, P); };
  auto run_cuda_plus_d2h = [&]() {
    solve(d_input, d_output, N, C, H, W, K, S, P);
  ...
  run_cuda();
  cuda_check(cudaMemcpy(out_gpu.data(), d_output, sizeof(float) * out_gpu.size(), cudaMemcpyDeviceToHost), "D2H output");
  const double diff_omp = max_abs_diff(out_single, out_omp);
  const double diff_gpu = max_abs_diff(out_single, out_gpu);
  if (diff_omp > 1e-6) {
    std::cerr << "FAIL OMP diff=" << diff_omp << " for NCHW=" << N << "x" << C << "x" << H << "x" << W
              << " K=" << K << " S=" << S << " P=" << P << "\n";
    std::exit(1);
  }
  if (diff_gpu > 1e-6) {
    std::cerr << "FAIL CUDA diff=" << diff_gpu << " for NCHW=" << N << "x" << C << "x" << H << "x" << W
              << " K=" << K << " S=" << S << " P=" << P << "\n";
    std::exit(1);
  }
  ...
  const double cuda_ms = best_ms(run_cuda, warmup, iters);
  const double cuda_total_ms = best_ms(run_cuda_plus_d2h, warmup, iters);
  ...
  std::cout << "NCHW=" << N << "x" << C << "x" << H << "x" << W
            ...
            << " omp_speedup=" << std::fixed << std::setprecision(2) << (single_ms / omp_ms)
            << " cuda_kernel=" << std::fixed << std::setprecision(3) << cuda_ms << " ms"
            << " cuda_speedup=" << std::fixed << std::setprecision(2) << (single_ms / cuda_ms)
            << " cuda_plus_D2H=" << std::fixed << std::setprecision(3) << cuda_total_ms << " ms"
            << "\n";
